...
The boot code is represented as a text file with one instruction per line of text. Each _instruction_ consists of an _operation_ (`acc`, `jmp`, or `nop`) and an _argument_ (a signed number like `+4` or `-20`).
...
Immediately _before_ the program would run an instruction a second time, the value in the accumulator is `5`.
